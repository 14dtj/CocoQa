...
def f1_max_over_ground_truths(metric_fn, prediction, ground_truths):
    scores_for_ground_truths = []
    for ground_truth in ground_truths:
        #print(metric_fn(prediction, ground_truth))
        score, precision, recall = metric_fn(prediction, ground_truth)
        scores_for_ground_truths.append((score,precision,recall))
        #print(precision,recall)


    #print(max(scores_for_ground_truths))
    return max(scores_for_ground_truths)
    ...
    for article in dataset:
        for paragraph in article['paragraphs']:
            for qa in paragraph['qas']:
                ...
                tmpf1, tmppre, tmprec = f1_max_over_ground_truths(
                    f1_score, prediction, ground_truths)
                #f1 += f1_max_over_ground_truths(
                #    f1_score, prediction, ground_truths)
                f1 += tmpf1
                precision += tmppre
                recall += tmprec

    # word-level的prec 和 recall与f1不同
