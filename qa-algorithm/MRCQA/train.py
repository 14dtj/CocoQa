...
def train(config_path):
    ...
    if model_choose == 'base':
        ...
        model = MatchLSTMPlus(dataset_h5_path, global_config['preprocess']['use_domain_tag'])
    ...
    if global_config['global']['finetune']:
        batch_train_data = dataset.get_dataloader_train2(train_batch_size, num_workers)
        batch_dev_data = dataset.get_dataloader_dev2(valid_batch_size, num_workers)
        for epoch in range(global_config['train']['finetune_epoch']):
            ...
            with torch.no_grad():
                ...
                valid_score_em, valid_score_f1, valid_loss = eval_on_model(model=model,
                                                                           ...
                                                                           device=device)
                valid_avg = (valid_score_em + valid_score_f1) / 2
            logger.info("finetune epoch=%d, ave_score_em=%.2f, ave_score_f1=%.2f, sum_loss=%.5f" %
                        (epoch, valid_score_em, valid_score_f1, valid_loss))
    ...
    if global_config['global']['finetune2']:
        batch_train_data = dataset.get_dataloader_train3(train_batch_size, num_workers)
        batch_dev_data = dataset.get_dataloader_dev3(valid_batch_size, num_workers)
        for epoch in range(global_config['train']['finetune_epoch2']):
            ...
            sum_loss = train_on_model(model=model,
                                      ...
                                      clip_grad_max=clip_grad_max,
                                      device=device)
            logger.info('finetune2 epoch=%d, sum_loss=%.5f' % (epoch, sum_loss))
            ...
            with torch.no_grad():
                ...
                valid_score_em, valid_score_f1, valid_loss = eval_on_model(model=model,
                                                                           ...
                                                                           device=device)
                valid_avg = (valid_score_em + valid_score_f1) / 2
            logger.info("finetune2 epoch=%d, ave_score_em=%.2f, ave_score_f1=%.2f, sum_loss=%.5f" %
                        (epoch, valid_score_em, valid_score_f1, valid_loss))
